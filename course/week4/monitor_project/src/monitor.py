...
def get_hist_score(tr_probs, te_probs, bins=100):
  score = None
  # ============================
  # FILL ME OUT
  # 
  ...
  tr_heights, tr_bins = np.histogram(tr_probs, bins=bins, density=True)
  te_heights, _ = np.histogram(te_probs, bins=tr_bins, density=True)
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    tr_probs_cal = cal_model.fit_transform(tr_probs, tr_labels)
    te_probs_cal = cal_model.transform(te_probs)
    tr_prob_cal = torch.from_numpy(tr_probs_cal).float()
    te_prob_cal = torch.from_numpy(te_probs_cal).float()
    # ============================
